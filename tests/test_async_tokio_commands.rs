...
fn test_graph_profile() {
    let res = create_runtime().block_on(issue_graph_profile_query("test_graph_profile_std"));
    check_graph_profile(res);
}

#[test]
fn test_graph_slowlog() {
    let res = create_runtime().block_on(issue_graph_slowlog_query("test_graph_slowlog_std"));
